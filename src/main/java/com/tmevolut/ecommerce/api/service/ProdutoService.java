...
public class ProdutoService {
    ...
    @Transactional
    @CacheEvict(value = "produto-individual", allEntries = true)
    ...
    public ProdutoResponse atualizarParcial(Long id, ProdutoPatchRequest request) {
        ...
        if (request.categoriaId() != null)
            p.setCategoria(categoriaService.buscarEntidade(request.categoriaId()));
    ...
    public void remover(Long id) {
        Produto p = buscarEntidade(id);
        p.softDelete(); // Garanta que este método exista na entidade Produto
        repository.save(p);
    }
